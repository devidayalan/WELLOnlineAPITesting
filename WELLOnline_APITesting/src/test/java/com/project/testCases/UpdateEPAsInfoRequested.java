...
public class UpdateEPAsInfoRequested extends TestBase {
	
	@Test
	public void UpdateEPAsAuthenticatedUser() throws IOException {
		
		JSONObject projObj = new JSONObject();
		constructJSONForEP(projObj);
	...
	public void UpdateEPAsAdmin() throws IOException {
		...
		String jsonString = projObj.toJSONString();
		 System.out.println("jsonsring is"+jsonString);
		 String aapId =  ExcelParserUtils.getSingleCellData(loginUserfile_path, emailListSheet, "EPId", 2);
		 res = given()
					.contentType("application/json")
